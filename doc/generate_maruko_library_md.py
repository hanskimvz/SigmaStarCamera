# -*- coding: utf-8 -*-
"""Generate Maruko library markdown from CSV paths + curated descriptions."""
...
# 출처: 사용자 제공 레퍼런스(Excel/CSV 스크린샷) 및 공식 표기에 가까운 설명
CURATED: dict[str, tuple[str, str]] = {
    ...
    "libMD_LINUX.so": (
        "基于IVE算子库，提供运动侦测的算法模块",
        "IVE 연산 기반 움직임 검출(MD)",
    ),
    "libOD_LINUX.so": (
        "基于IVE算子库，提供遮挡侦测的算法模块",
        "IVE 연산 기반 가림(occlusion) 검출(OD)",
    ),
    "libSRC_LINUX.so": ("重采样", "오디오 리샘플링"),
    "libSSL_2MIC_LINUX.so": ("声源定位", "음원 위치 추정(SSL, 2마이크)"),
    "libSSL_4MIC_LINUX.so": ("声源定位", "음원 위치 추정(SSL, 4마이크)"),
    "libVG_LINUX.so": (
        "基于IVE算子库，提供入侵报警策略的算法模块",
        "IVE 연산 기반 침입 경보(VIG) 전략 알고리즘",
    ),
    "libVIG_LINUX.so": (
        "基于IVE算子库，提供入侵报警策略的算法模块",
        "IVE 연산 기반 침입 경보(VIG) 전략 알고리즘",
    ...
    "libcam_os_wrapper.so": (
        "封装通用系统接口，供前端 cam(Oshm) 调用",
        "공통 OS API 래퍼(프론트엔드 cam/Oshm 연동)",
    ),
    "libcust.so": ("为客户提供结算的接口库", "고객 정산·맞춤용 인터페이스 라이브러리"),
    "libcus3a.so": ("为客户提供3A相关接口", "고객용 3A(AE/AWB/AF 등) 관련 인터페이스"),
    "libfbc_decode.so": (
        "MI layer 10bit 解码；IQ tool dfview 图形显示调用",
        "MI 레이어 10bit 디코딩(IQ 도구 dfview 등에서 사용)",
    ),
    "libg711.so": ("G711 编解码", "G.711 코덱"),
    "libg726.so": ("G726 编解码", "G.726 코덱"),
    "libispalgo.so": ("ISP 控制库算法库", "ISP 제어·알고리즘 라이브러리"),
    "libmi_ai.so": (
        "音频输入(Audio Input)；采集、发送及音频算法处理",
        "오디오 입력(AI): 디바이스 연동·전송·음성 알고리즘 처리",
    ),
    "libmi_ao.so": (
        "音频输出(Audio Output)；播放及音频算法处理",
        "오디오 출력(AO): 재생·음성 알고리즘 처리",
    ),
    "libmi_common.so": ("MI 通用功能模块", "MI 공통 모듈"),
    "libmi_disp.so": (
        "视频显示单元；图层合成与 HDMI/VGA/MIPI/TTL 输出",
        "디스플레이(DISP): 레이어 합성·HDMI/VGA/MIPI/TTL 출력",
    ),
    "libmi_dummy.so": ("Kernel 层测试 mi_sys", "커널 측 mi_sys 테스트용 더미 모듈"),
    "libmi_fb.so": (
        "Linux Framebuffer 显示；缓冲区与 Alpha/ColorKey 等",
        "프레임버퍼(FB): 버퍼 관리·Alpha·ColorKey·영역 설정",
    ),
    "libmi_gfx.so": (
        "GFX 图形引擎硬件加速；绘制、缩放、旋转、Alpha、Color Key 等",
        "GFX: UI 그래픽 가속(그리기·스케일·회전·알파·컬러키 등)",
    ),
    ...
    "libmi_isp.so": (
        "图像信号处理；HDR、3D/2D 降噪、3A、WDR 等",
        "ISP: HDR·3D/2D NR·3A·WDR 등 영상 신호 처리",
    ),
    "libmi_ive.so": ("视频智能分析算子引擎基础支持", "영상 지능 분석(IVE) 기본 연산"),
    "libmi_jpd.so": (
        "JPD 硬解码；拍照、对讲等场景",
        "JPD 하드웨어 디코드(캡처·인터컴 등)",
    ),
    "libmi_ldc.so": ("镜头畸变校正", "렌즈 왜곡 보정(LDC)"),
    "libmi_mipi_tx.so": ("MIPI TX", "MIPI TX 송신"),
    "libmi_mipitx.so": ("MIPI TX", "MIPI TX 송신"),
    "libmi_panel.so": ("点屏 / Panel", "패널 구동"),
    "libmi_pq.so": (
        "通过 ISP/sensor 获取图像并进行 PQ 画质处理",
        "ISP·센서 기반 PQ(화질) 처리",
    ),
    ...
    "libmi_vdisp.so": (
        "Virtual Display：多路 YUV/RGB 拼接整幅画面",
        "가상 디스플레이: 다채널 YUV/RGB 합성",
    ),
    "libmi_venc.so": (
        "视频编码；H.264/H.265 参数配置与查询",
        "비디오 인코더(VENC): H.264/H.265 설정·조회",
    ),
...
def read_csv_first_column(csv_path: pathlib.Path) -> list[str]:
    """CSV는 따옴표·쉼표·줄바꿈이 섞일 수 있어 csv.reader로 첫 열만 안전하게 읽는다."""
...
def main() -> None:
    default_csv = pathlib.Path(
        "/home/hanskim/Dev_Board_doc_sdk_release/SDK_image_and_tools/"
        "TOOLS_SDK_202030831/Maruko-ILS00_TINY_V1.2.0/reference(excel)/"
        "Library description/Maruko库功能说明.csv"
    )
    csv_path = pathlib.Path(sys.argv[1]) if len(sys.argv) > 1 else default_csv
    out_path = pathlib.Path(sys.argv[2]) if len(sys.argv) > 2 else pathlib.Path(
        "/mnt/d/Projects/starLightCamera/doc/Maruko_라이브러리_기능_설명.md"
    )
    ...
    names = read_csv_first_column(csv_path)
    libs_so = [n for n in names if n.endswith(".so")]
    libs_ko = [n for n in names if n.endswith(".ko")]
    other = [n for n in names if not (n.endswith(".so") or n.endswith(".ko"))]

    lines: list[str] = []
    lines.append("# Maruko SDK 라이브러리·커널 모듈 기능 설명")
    lines.append("")
    lines.append("## 출처")
    lines.append("")
    lines.append(
        "- 원본 CSV: `Maruko库功能说明.csv`  "
        "(예: `TOOLS_SDK_202030831/Maruko-ILS00_TINY_V1.2.0/reference(excel)/Library description/`)"
    )
    lines.append(
        "- 본 문서의 **기능 설명**은 동일 자료의 레퍼런스 표·공개 요약을 바탕으로 정리했습니다."
    )
    lines.append(
        "- 원본 CSV 파일의 한글/중국어 본문은 저장 인코딩 문제로 자동 추출이 불안정할 수 있어, "
        "**파일명 목록은 CSV와 동일**하게 두고 설명은 정제본만 표에 넣었습니다."
    )
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append("## 사용자 공간 라이브러리 (`.so`)")
    lines.append("")
    lines.append("| 파일 | 원문 요약(중국어) | 한국어 설명 |")
    lines.append("| --- | --- | --- |")
    for n in libs_so:
        if n in CURATED:
            zh, ko = CURATED[n]
            lines.append(
                f"| `{escape_md_cell(n)}` | {escape_md_cell(zh)} | {escape_md_cell(ko)} |"
            )
        else:
            lines.append(f"| `{escape_md_cell(n)}` | — | *(CSV에 설명 필드 없음 또는 미정리)* |")
    lines.append("")
    lines.append("## 커널 모듈 (`.ko`)")
    lines.append("")
    lines.append("| 파일 | 원문 요약(중국어) | 한국어 설명 |")
    lines.append("| --- | --- | --- |")
    for n in libs_ko:
    ...
    lines.append("---")
    lines.append("")
    lines.append("## 부록: `libVG` vs `libVIG`")
    lines.append("")
    lines.append(
        "일부 문서에는 침입 경보 알고리즘 라이브러리가 `libVIG_LINUX.so` 로 표기되고, "
        "배포 CSV에는 `libVG_LINUX.so` 로만 올라 있는 경우가 있습니다. 빌드 산출물 이름을 기준으로 맞추면 됩니다."
    )
    lines.append("")

    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text("\n".join(lines), encoding="utf-8")
    print(f"Wrote {out_path} ({len(names)} rows)")
